...
export function VideoLibrary(): JSX.Element {
  ...
  const zeigen = async (id: string): Promise<void> => {
    try {
      await api('projection.setMode', { mode: 'video', videoId: id })
    } catch (fehler) {
      app.reportError(fehler)
    }
  }
      ...
      {video && (
        <div className="notice" style={{ marginBottom: 12 }}>
          <div className="row" style={{ alignItems: 'center', gap: 10 }}>
            <button
              className="primary"
              onClick={() => void api('video.setPlaying', !video.playing).catch(app.reportError)}
            >
            ...
            <button onClick={() => void api('video.setMuted', !video.muted).catch(app.reportError)}>
